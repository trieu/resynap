...
def simulate_jewellery_transactions_enhanced(
    ...
    for customer_id in range(num_customers):
        ...
        assigned_persona = random.choice(customer_segments[assigned_segment]) if customer_segments and assigned_segment in customer_segments and customer_segments[assigned_segment] else "Default"
        ...
        acquisition_cost = random.uniform(acquisition_cost_range, acquisition_cost_range[1])
        is_active = True
        months_active = 0
        total_spending = 0
        transactions =
        ...
        for month in range(1, simulation_duration_months + 1):
            ...
            if persona_category_probabilities and assigned_persona in persona_category_probabilities:
                ...
                purchase_category = random.choices(possible_categories, weights=list(category_probs.values()), k=1)

                # Adjust base probability based on persona's general purchase tendency
                if "purchase_frequency_multiplier" in customer_personas[assigned_persona]:
                ...
                if "average_order_value_multiplier" in customer_personas[assigned_persona]:
            ...
            if random.random() < purchase_probability:
                ...
                transactions.append({"month": month, "amount": order_value, "category": purchase_category if 'purchase_category' in locals() else 'unknown'})
...
if not customer_data_row.empty:
    transactions = customer_data_row["transactions"].iloc
    if transactions:
        print(f"\nTransactions for Customer {customer_id_to_check} (Segment: {customer_data_row['assigned_segment'].iloc}, Persona: {customer_data_row['assigned_persona'].iloc}):\n")
...
# 7. Active customers
active_customers_enhanced = enhanced_simulation_results_df[enhanced_simulation_results_df["is_active"] == True].shape
print(f"\nNumber of Active Customers at Simulation End (Enhanced): {active_customers_enhanced}")

# 8. churned customers
churned_customers_enhanced = enhanced_simulation_results_df[enhanced_simulation_results_df["is_active"] == False].shape
